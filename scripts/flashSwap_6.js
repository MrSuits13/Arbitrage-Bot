...
async function main() {
  ...
  Flash = await ethers.getContractFactory("FlashSwap", signer2);
  ...
  let usdtBalance = await usdtContract
    .connect(provider)
    .balanceOf(signer2.address);
  let usdcBalance = await usdcContract
    .connect(provider)
    .balanceOf(signer2.address);
  console.log("-------------------- BEFORE");
