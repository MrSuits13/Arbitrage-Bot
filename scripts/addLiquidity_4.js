...
async function main() {
  ...
  await usdtContract
    .connect(owner)
    .approve(POSITION_MANAGER_ADDRESS, ethers.utils.parseEther("9999999"));
  await usdcContract
    .connect(owner)
    .approve(POSITION_MANAGER_ADDRESS, ethers.utils.parseEther("9999999"));
  ...
  const poolContract1 = new Contract(
    USDT_USDC_500,
    artifacts.UniswapV3Pool.abi,
    provider
  );
  const poolContract2 = new Contract(
    USDT_USDC_3000,
    artifacts.UniswapV3Pool.abi,
    provider
  );
  const poolContract3 = new Contract(
    USDT_USDC_10000,
    artifacts.UniswapV3Pool.abi,
    provider
  );
  ...
  await Promise.all(
    POOL_ADDRESSES.map(async (poolAddress) => {
      ...
      const tickLower =
        nearestUsableTick(pd.tick, pd.tickSpacing) - pd.tickSpacing * 100;
      const tickUpper =
        nearestUsableTick(pd.tick, pd.tickSpacing) + pd.tickSpacing * 100;
  ...
  const tx1 = await nonfungiblePositionManager
    .connect(owner)
    .mint(mintParams[USDT_USDC_500], { gasLimit: "1000000" })
    .catch((error) => {
      console.error("Error in minting USDT_USDC_500:", error);
      throw error;
    });
  await tx1.wait();

  const tx2 = await nonfungiblePositionManager
    .connect(owner)
    .mint(mintParams[USDT_USDC_3000], { gasLimit: "1000000" })
    .catch((error) => {
      console.error("Error in minting USDT_USDC_3000:", error);
      throw error;
    });
  await tx2.wait();

  const tx3 = await nonfungiblePositionManager
    .connect(owner)
    .mint(mintParams[USDT_USDC_10000], { gasLimit: "1000000" })
    .catch((error) => {
      console.error("Error in minting USDT_USDC_10000:", error);
      throw error;
    });
  await tx3.wait();
  console.log("done");
